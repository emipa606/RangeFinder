...
internal static class Tools
{
    ...
    public static bool IsModKeyUp(this ModifierKey key)
    {
        return key switch
        {
            ...
            ModifierKey.Meta => Input.GetKeyUp(KeyCode.LeftWindows) || Input.GetKeyUp(KeyCode.RightWindows)
                                                                    ...
                                                                    || Input.GetKeyUp(KeyCode.LeftApple) ||
                                                                    Input.GetKeyUp(KeyCode.RightApple),
            _ => false
        };
    }
    ...
    {
        return Find.Selector.SelectedObjects.Where(obj => obj is IAttackTargetSearcher)
            .Cast<IAttackTargetSearcher>()
            .Where(targetSearcher =>
            {
                if (targetSearcher.Thing == null || !targetSearcher.Thing.Spawned)
                {
                    return false;
                }
    ...
    public static void ValueLabeled<T>(this Listing_Standard listing, string name, ref T value, string tooltip = null)
    {
        ...
        if (typeof(T).IsEnum)
        {
            Widgets.Label(rect, (typeof(T).Name + "Option" + value).Translate());
        }
        else
        {
        ...
        string rightAlignedLabel = null, float roundTo = -1f)
    {
        ...
        var num = GUI.HorizontalSlider(rect, value, leftValue, rightValue);
        if (!label.NullOrEmpty() || !leftAlignedLabel.NullOrEmpty() || !rightAlignedLabel.NullOrEmpty())
        {
            var anchor = Text.Anchor;
            var font = Text.Font;
            Text.Font = GameFont.Tiny;
            var num2 = label.NullOrEmpty() ? 18f : Text.CalcSize(label).y;
